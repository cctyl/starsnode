...
void getCpuCountAndName(CpuInfo &c) {
    ...
    while (!feof(fp)) {
        ...
        if (is_begin_with(szTest, const_cast<char*>("model name")) && strlen(modelName) < 1) {

            sscanf(szTest, "%*[^:]: %[^\n]", modelName);
            //printf("b1=%s,\n",modelName);
            //printf("%s", szTest);
            c.cpuModel = std::string(modelName);
        }


        if (is_begin_with(szTest, const_cast<char*>("cpu cores")) && strlen(cpuCoreNum) < 1) {
...
void Info::osInfo() {
    ...
    d.osInfo.type = buf.sysname;
    d.osInfo.platform = "linux";
    d.osInfo.release = std::string(buf.release).append(" ")
            .append(std::string(buf.version));

    d.osInfo.hostname = localmachineName();
    d.osInfo.arch = buf.machine;
...
void read_net_stats(NetStats *stats, int max_interfaces) {
    ...
    while (fgets(line, sizeof(line), fp) && count < max_interfaces) {
        ...
        sscanf(line, "%15s %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu",
...
void calculate_rates(
    ...
    for (int i = 0; i < INTERFACE_NUM; i++) {
        ...
        totalRx += rx_rate;
        totalTx += tx_rate;
        netstatInfo[std::string(new_stats[i].name)] = {
